...
(set! tmp-str (string-append program-name " help/usage (" help-opt ")"))
(sprintf node-fmt "usage" tmp-str) =]
...
@menu
[=
IF  (ag-fprintf 0 "* %s %-24s %s help/usage (%s)\n"
        down-prog-name "usage::" program-name help-opt)
    have-doc-options                    =][=

  (define menu-entry-fmt  "* %s %-24s %s options")
  (define emit-menu-entry (lambda (is-doc) is-doc))  =][=

ELSE                                    =][=
  (define menu-entry-fmt  "* %s %-24s %s option")
  (define emit-menu-entry (lambda (is-doc) (not is-doc)))  =][=
ENDIF                                   =][=

FOR flag                                =][=

  IF (emit-menu-entry (exist? "documentation"))
  =][=

 (define opt-name (string-tr! (get "name") "A-Z^_" "a-z--"))
 (sprintf menu-entry-fmt down-prog-name
