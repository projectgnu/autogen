/*  -*- Mode: C -*-
 *
 *  $Id: getdefs.c,v 2.30 2001/09/21 03:09:48 bkorb Exp $
 *
 ...
 *  Last Modified:     $Date: 2001/09/21 03:09:48 $
...
compar_func compar_text, compar_defname;

char* assignIndex( char*  pzOut,  char*  pzDef );
int awaitAutogen( void );
void buildDefinition(
    char*    pzDef,
    char*    pzFile,
    int      line,
    char*    pzOut );
tSuccess buildPreamble(
    char**   ppzDef,
    char**   ppzOut,
    char*    pzFile,
    int      line );
void compressDef( char* pz );
void doPreamble( FILE* outFp );
char* emitDefinition( char* pzDef, char* pzOut );
char* emitQuote( char** ppzText, char* pzOut );
char* emitSubblock( char* pzDefList, char* pzText, char* pzOut );
char* emitSubblockString( char** ppzText, char sepChar, char* pzOut );
char* loadFile( char* pzFname );
void loadStdin( void );
void printEntries( FILE* fp );
void processFile( char* pzFile );
void setFirstIndex( void );
